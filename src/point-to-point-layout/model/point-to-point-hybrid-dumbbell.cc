...
#include "ns3/point-to-point-hybrid-dumbbell.h"

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("PointToPointHybridDumbbellHelper");

PointToPointHybridDumbbellHelper::PointToPointHybridDumbbellHelper (uint32_t nLeftLeaf,
                                                                    uint32_t nRightLeaf,
...
PointToPointHybridDumbbellHelper::~PointToPointHybridDumbbellHelper ()
{

}

Ptr<Node> PointToPointHybridDumbbellHelper::GetLeft () const
{ // Get the left side bottleneck router
  return m_centralNodes.Get (0);
}

Ptr<Node> PointToPointHybridDumbbellHelper::GetLeft (uint32_t i) const
{ // Get the i'th left side leaf
  return m_leftLeaf.Get (i);
}

Ptr<Node> PointToPointHybridDumbbellHelper::GetRight () const
{ // Get the right side bottleneck router
  return m_centralNodes.Get (2);
}

Ptr<Node> PointToPointHybridDumbbellHelper::GetRight (uint32_t i) const
{ // Get the i'th right side leaf
  return m_rightLeaf.Get (i);
}

Ptr<Node> PointToPointHybridDumbbellHelper::GetMiddle () const
{
  return m_centralNodes.Get (1);
}

Ipv4Address PointToPointHybridDumbbellHelper::GetLeftIpv4Address (uint32_t i) const
{
  return m_leftLeafInterfaces.GetAddress (i);
}

Ipv4Address PointToPointHybridDumbbellHelper::GetRightIpv4Address (uint32_t i) const
{
  return m_rightLeafInterfaces.GetAddress (i);
}

uint32_t  PointToPointHybridDumbbellHelper::LeftCount () const
{ // Number of left side nodes
  return m_leftLeaf.GetN ();
}

uint32_t  PointToPointHybridDumbbellHelper::RightCount () const
{ // Number of right side nodes
  return m_rightLeaf.GetN ();
}

void PointToPointHybridDumbbellHelper::InstallStack (InternetStackHelper stack)
{
...
void PointToPointHybridDumbbellHelper::AssignIpv4Addresses (Ipv4AddressHelper leftIp,
